...
企業内でクローズされたModelicaのライブラリを対象にRAGで関連ドキュメントを検索し，プランナー，コーダ，テスターの三つのエージェントが協調してライブラリを自動組立するフレームワークを構築した事例を報告する．OpenAI 4o-miniを用いた実験ではRAGでけのシステムでは，生成できなかったコード作成タスクにおいて候補コードが生成できるようになることを確認できた．
...
企業内では，社内でクローズされた情報を検索し設計活動に用いる．このように最近は，機密情報をRAGなどの検索システムを構築し，設計知識管理や情報の共有にLLMが活用されている．

===　物理シミュレーションの支援
物理シミュレーションの支援に関するLLMの適用事例を記す。
シミュレーションは，モデルを集約した専門ライブラリを使いユーザがシミュレーションモデルを構成する．
近年の大規模言語モデルLLMはコード生成力の向上により，Modelica コンポーネント自動生成への応用が進む．ベンチマーク「ModiGen」では，LLM が  指標で 0.33 以上を達成する事例が報告されており，複雑なモデル構造でも一定の妥当性を保持する段階に到達した#citep(<xiang2025modigenlargelanguagemodelbased>)．
数値流体シミュレーションでよく使われるOpenFOAMは，C＋＋言語で作成されたライブラリである．
OpenFOAMを用いたCFDシミュレーションの自動化を目的として，GPT-4oおよびCoT機能搭載モデルを基盤とするエージェント「OpenFOAMGPT」が提案されている．本手法では，OpenFOAMチュートリアルケースから得た領域知識を検索拡張生成（RAG）パイプラインに組み込み，入力ファイルの自動生成・実行・エラー訂正を反復的に行う構造を採用している．評価では，RAGを用いた少数ショットプロンプトにより，境界条件やメッシュ解像度の変更に対応できることが示された．一方，精度担保のための人手監視やモデル性能の時間変動が課題として指摘されている．「MetaOpenFOAM」は，Architect，InputWriter，Runner，Reviewerの四つのエージェントが連携し，ユーザーの自然言語要求から入力ファイル生成，シミュレーション実行，エラー分析・修正までを自動化する反復ループを構築する．

先行研究ではLLMによる完全自動化を試行しているが，いまだにLLMによるシミュレーションの完全自動化は難しい．完全自動化が困難な理由を列挙すると以下の通りである．
...
- 信頼性と正確性　シミュレーション作業のセットアップは，一般的に厳密な正確性が要求される．モデリング作業は，複数のセットアップ作業を正確に実行する必要があるが，すべての作業を正確にじっこうすることは現在のLLMにとっては，難しい．
- ライブラリ機密性　企業の研究開発に用いる専門ライブラリは，機密性が高く，インターネットには公開されていない．LLMの訓練データにはいっさい含まれていないため，LLMに専門ライブラリの情報を与えるために，RAGによって検索結果をLLMに与えることが一般的である．専門ライブラリの活用は，一企業内だけにとどまることがほとんどのため，ファインチューニングに必要な訓練データを用意することも難しい．
- ベンチマーク問題の作成　LLMの回答精度を向上させるためには，システムプロンプトの工夫やパラメータのチューニングが必要である．専門ライブラリは，ユーザ数が限られており，専門性の高い知識が必要でありるため，LLMをチューニングするためのベンチマーク問題を用意することが難しい．
...
各領域のオーバーラップ部分をシミュレーションモデルを使い効率的に開発するための技術体系は，モデルベース開発（MBD）と呼ばれている．モデルベース開発では，メカ領域は3次元CADをベースとしたCAEによって製品の挙動や性質を分析する一方で，電気や制御は集中乗数系のシミュレーションで動的な挙動を分析することが多い．
1DCAEとは，MBDの手法の一つであり、3次元のメカ領域を抽象化し、電気、ソフトなどのシミュレーションに合わせ，機械系モデルを含めた複合領域の統合シミュレーションを可能にする。1DCAEにより複合領域のシミュレーションが可能になり，コンカレントエンジニアリングの実現を支援する．

===　設計カタログとしてのライブラリ

1DCAEのモデルは，ライブラリとして構成されている．Modelicaライブラリには，対象システムの動的挙動を再現するための理論と実験データが集積されており，モデルの忠実度（Fidelity）を支える根拠として機能する．
...
完成度の高い1DCAEのライブラリは，対象システムを物理法則やその根拠となるデータを備えており，設計カタログとしての性質を備えているといえる．
