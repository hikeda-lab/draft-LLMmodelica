...
#let jsme_init(body) = {
  ...
  set table(stroke: (top: 0.5pt + black, bottom: 0.5pt + black, right: none, left: none), align: (x, y) => (
    if x > 0 { center } else { left }
  ))
  show figure.where(
    // if figure kind is table ...
    kind: table,
  ...
  show figure.caption: it => {
    ...
    grid(
      columns: 2,
      [#supplement #context it.counter.display()　], align(left)[#it.body],
    )
  }
  //setting equation
...
#let jsme_title(
  ...
  footnote(
    "[No.25-10] 日本機械学会 第35回設計工学・システム部門講演会講演論文集〔2025.9.3.-5 明治大学アカデミーコモン（東京都千代田区）〕",
  )
  ...
  //setting abstruct
  if abstruct != [] {
    text(font: english, weight: "bold")[Abstract\ ]
    abstruct
  }
  v(1em)
